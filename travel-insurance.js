window.onload = function () {
    ...
    function fetchPolicies(page, emailSearch = '') {
        let apiUrl = 'http://localhost:5555/restv2/insurance/getpolicybytypes?input=Travel Insurance';
    ...
    // Function to validate and format the Policy No input (only allow numbers after "TI-")
    function validatePolicyNo() {
        const input = document.getElementById('policyNoSearch');
        let value = input.value;

        // Make sure the input always starts with "TI-"
        if (!value.startsWith('TI-')) {
            input.value = 'TI-';
        } else {
            // Allow only numbers after the "TI-" part
            const numericPart = value.slice(3).replace(/[^0-9]/g, ''); // Remove non-numeric characters
            input.value = 'TI-' + numericPart; // Reassign the value back to the input
        }
    }

    // Function to search policies by Policy No
    function searchPolicyByNo(policyNo) {
        if (!policyNo.startsWith('TI-') || policyNo.length <= 3) {
            alert('Please enter a valid policy number starting with "TI-" followed by digits.');
            return;
        }
    ...
    // Form Submission for New Policy
    document.getElementById('travelInsuranceForm').addEventListener('submit', function (event) {
        ...
        if (!name || !email || !mobile || !dob || !status || !premium || !destination || !departure || !fromDate || !toDate || !aadhar) {
        ...
        fetch('http://localhost:5555/restv2/insurance/inserttravel', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        .then((data) => {
            const successMessage = data.message || 'Policy added successfully!';
        
            alert(successMessage);
            fetchPolicies(currentPage);

            document.getElementById('travelInsuranceForm').reset();
